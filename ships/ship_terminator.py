...
class ShipTerminator(BaseShip):
    def __init__(self, x, y, color):
        ...
        # Параметры основного оружия (Ion cannon)
        self.weapon_energy_cost = 1
        self.weapon_wait = 0 / 60.0  # может стрелять очень быстро
        self.weapon_timer = 0

        # Параметры специального оружия (Shield)
        ...
        self.turn_speed = 90.0  # значение можно корректировать по необходимости

        self.shield_timer = 0  # время действия щита (неактивен по умолчанию)

    def fire_missile(self, enemy, game_time):
        """
        Ion cannon: стреляет двумя пульсными ракетами (по одной с каждого крыла).
        Ракеты не являются гоминг-снарядами и летят прямо вперёд.
        Каждая ракета наносит 1 единицу урона.
        """
        if self.weapon_timer <= 0 and self.energy >= self.weapon_energy_cost:
            ...
            # Вычисляем вектор вперёд
            forward_x = math.sin(rad)
            forward_y = -math.cos(rad)
            # Вычисляем правый вектор (перпендикуляр к направлению вперёд)
            right_x = math.cos(rad)
            right_y = math.sin(rad)
            # Отступ от центра: вперед на расстояние равное радиусу,
            # и смещение по горизонтали (относительно направления движения) на величину радиуса
            ...
            # Дополнительная скорость ракет (они летят вперёд)
            missile_speed = 50.0
            missile_vx = self.vx + forward_x * missile_speed
            missile_vy = self.vy + forward_y * missile_speed
            # Импортируем класс Missile (предполагается, что он корректно обрабатывает target=None как не гоминг)
    ...
    def fire_laser_defense(self, game_time):
        """
        Shield activation: при активации (rshift/q) щит включается, расходуя 3 единицы энергии.
        Щит действует в течение 0.5 секунд, в течение которых корабль не получает урон.
        """
        if self.special_timer <= 0 and self.energy >= self.special_energy_cost:
            ...
            self.shield_timer = 0.5  # длительность действия щита (0.5 сек)
        return []
    ...
    def take_damage(self, amount):
        # Если щит активен, урон не наносится
        if self.shield_timer > 0:
            return
